...
const StripeForm = ({ isScriptLoaded, isScriptLoadSucceed }) => {
  ...
  React.useEffect(() => {
    if (isScriptLoaded && isScriptLoadSucceed) {
      setStripe(window.Stripe('pk_test_Llnv3emOhx6kls4GSRDqLxVr009Mw1CZE9'));
    }
  ...
  const handleSubmit = async event => {
    ...
    const result = await stripe.redirectToCheckout({
      items: [
        {
          plan: 'plan_F3BMYOA8oqXtWG',
          quantity: 1
        }
      ],
      successUrl: 'https://www.energiaemobilitasostenibile.org/success',
